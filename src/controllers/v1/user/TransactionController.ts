...
export class TransactionController {
	...
	public async inquiryTransferToUser(@Req() request: Req): Promise<{ target: User }> {
		...
		receiver.full_name = receiver.full_name
			.split(' ')
			.map((nameByWord) =>
				nameByWord
					.split('')
					.map((char, index) => (index === 0 || index === 1 || index === nameByWord.length - 1 ? char : '*'))
					.join('')
			)
			.join(' ');
	...
	@UseAuth(UserAuthenticationMiddleware)
	public async confirmTransferToUser(
		@Req() request: Req
	): Promise<{
		user: User;
		...
		try {
			...
			await Promise.all([this.manager.save(senderWalletHistory), this.manager.save(receiverWalletHistory)]);
	...
	@UseAuth(UserAuthenticationMiddleware)
	public async confirmBankTransfer(
		@Req() request
	): Promise<{
		user: User;
		transaction: Transaction;
		...
		try {
			...
			const result = await Promise.all([this.manager.save(transaction), this.manager.save(user)]);
	...
	public async instantTopup(
		...
		try {
			...
			transaction.wallet_type = WalletType.CASH;
			user.current_cash = user.current_cash + body.amount;
			const results = await Promise.all([this.manager.save(transaction), this.manager.save(user)]);
	...
	public async inquiryPlnPrepaidPayment(
		...
		const response = await axios.post<PLNPrepaidSubscriptionData>(
	...
	public async inquiryPlnPostpaidPayment(
		...
		const reference_id = uuidv1();
		const response = await axios.post<PLNPostpaidSubscriptionData>(
